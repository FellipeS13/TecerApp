...
const HomeScreen = ({ navigation }) => {
  ...
  const getDocuments = async () => {
      const doc = await fetch('http://52.67.83.67:3000/obterDocumentosApp', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({
            clienteID: globalVariable,
            categoriaDOC: 11
            
          }),
      });
    const docs = await doc.json();
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.logoContainer}>
        ...
        <Text style={styles.username}>Documentos Pessoa Jurídica</Text>
      </View>
      ...
      <View style={styles.cardsContainer}>
        ...
        <TouchableOpacity style={styles.cardButton} onPress={() => back()}>
